...
export const obtenerClientesPaginados = async (page: number = 1, limit: number = 10, busqueda: string = "") => {
  try {
    ...
    const response = await api.get<PaginatedResponse<any>>(`/usuarios`, { params });
...
export const registrarCliente = async (clienteData: any) => {
  const response = await api.post("/usuarios", clienteData);
  return response.data;
};
