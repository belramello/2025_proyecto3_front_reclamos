...
import { Trash2, ChevronLeft, ChevronRight, Briefcase, Phone } from "lucide-react"; 
import { obtenerClientesPaginados, eliminarCliente } from "@/services/ClientesService";
              ...
              {clientes.length === 0 ? (
                ...
                clientes.map((cli) => (
                  <tr key={cli._id || cli.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-medium text-gray-900">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                {/* Mostramos la dirección pequeña abajo del nombre */}
                                <p className="text-xs text-gray-400">{cli.direccion || "Sin dirección"}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <span className={`badge ${cli.estado === 'Activo' ? 'bg-success' : 'bg-warning'}`}>
                        {cli.estado} 
                    </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <Button variant="ghost" size="icon" onClick={() => handleEliminar(cli)}>
