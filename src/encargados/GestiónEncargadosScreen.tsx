...
import { obtenerEncargadosPaginados, eliminarEncargado } from "@/services/EncargadoService";
...
export default function GestionEncargadosScreen() {
  ...
  const cargarEncargados = async () => {
    ...
    try {
      ...
      // LOG DE CONTROL: Aquí verás exactamente qué llega del back
  ...
  if (loading && page === 1) return <div className="p-6 flex justify-center"><Spinner /></div>;

  return (
    <div className="w-full h-full p-6 flex flex-col gap-6"> 
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full">
          <div>
            <h2 className="text-3xl font-bold tracking-tight text-gray-900">Encargados de Áreas</h2>
                  ...
                  <tr key={enc._id || enc.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="font-semibold text-gray-900 text-base">{enc.nombre}</p>
                    ...
                    <td className="px-6 py-4 text-gray-600">
                        <div className="flex flex-col gap-1">
                            <span className="flex items-center gap-2"><Mail size={14} className="text-gray-400"/> {enc.email}</span>
                    ...
                    <td className="px-6 py-4 text-center">
                        <Badge variant={enc.estado === 'Activo' ? 'default' : 'secondary'}>
                            {enc.estado || "Activo"}
                        </Badge>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <Button variant="ghost" size="icon" onClick={() => handleEliminar(enc)}>
